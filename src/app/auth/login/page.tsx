    ...
    <div className="min-h-screen bg-[#050505] text-white flex items-center justify-center p-4">
      <div className="bg-[#111] border border-[#222] w-full max-w-md p-8 rounded-2xl shadow-2xl relative overflow-hidden">
        ...
        <div className="text-center mb-8">
          <div className="inline-block p-3 rounded-full bg-blue-900/20 mb-4"><ShoppingBag className="text-blue-400 w-8 h-8" /></div>
        ...
        {errorMsg && <div className="mb-6 bg-red-900/10 border border-red-900/50 p-3 rounded-lg flex items-center gap-2 text-sm text-red-400"><AlertCircle size={16} /> {errorMsg}</div>}
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 top-3.5 text-gray-500 hover:text-white"><Eye size={18}/></button>
            </div>
          </div>
          ...
          <div className="text-center text-[10px] text-gray-700 mt-4 font-mono">MODO PRUEBA: cliente@demo.com / 123456</div>
          <p className="text-center text-gray-500 text-sm mt-4">¿Nuevo aquí? <Link href="/auth/register" className="text-blue-400 font-bold hover:underline">Crea una cuenta</Link></p>
        </form>
      </div>
    </div>
  );
